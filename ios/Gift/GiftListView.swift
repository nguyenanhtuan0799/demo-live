...
class GiftListView: UIView {
  ...
  private lazy var collectionView: UICollectionView = {
    let view = UICollectionView(
      frame: self.bounds, collectionViewLayout: self.flowLayout)
    if #available(iOS 11.0, *) {
      view.contentInsetAdjustmentBehavior = .never
    }
    view.register(
      TUIGiftCell.self,
      forCellWithReuseIdentifier: TUIGiftCell.cellReuseIdentifier)
  ...
  init(roomId: String, delegate: GiftListViewDelegate? = nil) {
    ...
    setupUI()
    print(giftDataSource, "??giftDataSource")
  }
...
extension GiftListView: UICollectionViewDelegate {
  func collectionView(
    ...
    if let index = currentSelectedCellIndex,
      let cell = collectionView.cellForItem(at: index) as? TUIGiftCell
    {
    ...
    if giftDataSource.count > indexPath.row {
      let selectedGift = giftDataSource[indexPath.row]
      showGiftDetailModal(for: selectedGift)
    }
  }

  private func showGiftDetailModal(for gift: TUIGift) {
    let modalView = GiftDetailModalView(gift: gift, delegate: self)
    modalView.frame = CGRect(x: -100, y: 0, width: 300, height: 300)
...
extension GiftListView: UICollectionViewDataSource {
  ...
  func collectionView(
    ...
    let reuseCell = collectionView.dequeueReusableCell(
      withReuseIdentifier: TUIGiftCell.cellReuseIdentifier, for: indexPath)
...
extension GiftListView {
  func sendGift(
    model: TUIGift, giftCount: Int, receiver: TUIGiftUser,
    completion: @escaping (_ isSuccess: Bool, _ message: String) -> Void
  ) {
    DataReporter.reportEventData(eventKey: getReportKey())
    manager.sendGift(model, receiver: receiver, giftCount: giftCount) {
      code, msg in
...
extension GiftListView {
  private func getReportKey() -> Int {
    ...
    case .liveRoom:
      key =
        isSupportEffectPlayer
        ? Constants.DataReport.kDataReportLiveGiftEffectSendCount
        : Constants.DataReport.kDataReportLiveGiftSVGASendCount
    case .voiceRoom:
      key =
        isSupportEffectPlayer
        ? Constants.DataReport.kDataReportVoiceGiftEffectSendCount
...
extension GiftListView: GiftDetailModalViewDelegate {

  func didTapCancel() {
    print("User cancelled sending gift")
    // Có thể thêm logic nếu cần
  }

  func didTapSendGift(gift: TUIGift) {
    print(gift, "??gift")
    delegate?.onSendGift(gift: gift, giftCount: 1)
  }
}
